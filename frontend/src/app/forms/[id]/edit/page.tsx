...
function EditFormContent() {
  ...
  const fetchForm = async () => {
    try {
      ...
      // Normalize fields: extract field_types and conditional logic recursively
                          ...
                          {(deeperField.field_types || []).length > 0 && (
                            <div className="mt-1 pt-1 border-t border-gray-200">
                              ...
                              {(deeperField.field_types || []).includes(FieldType.PHOTO) && (
                                <div className={`grid grid-cols-2 gap-1 mb-1 p-1 bg-gray-100 text-black rounded text-[${Math.max(6, fontSize - 2)}px] border border-gray-300`}>
                                  <div>
                                    <label className="block font-medium text-black mb-0.5">Max File Size (MB)</label>
                                    <input
                                      type="number"
                                      min="1"
                                      max="50"
                                  ...
                                  <div>
                                    <label className="block font-medium text-black mb-0.5">Image Quality</label>
                                    <select
                                      className={`w-full text-[${Math.max(6, fontSize - 2)}px] border border-gray-300 rounded px-1 py-0.5 text-black bg-white`}
                              ...
                              {(deeperField.field_types || []).includes(FieldType.MEASUREMENT) && (
                                <div className={`grid grid-cols-3 gap-1 mb-1 p-1 bg-gray-100 text-black rounded text-[${Math.max(6, fontSize - 2)}px] border border-gray-300`}>
                                  <div>
                                    <label className="block font-medium text-black mb-0.5">Measurement Type</label>
                                    <select
                                      className={`w-full text-[${Math.max(6, fontSize - 2)}px] border border-gray-300 rounded px-1 py-0.5 text-black bg-white`}
                                  ...
                                  <div>
                                    <label className="block font-medium text-black mb-0.5">Min Value</label>
                                    <input
                                      type="number"
                                      step="0.01"
                                      className={`w-full text-[${Math.max(6, fontSize - 2)}px] border border-gray-300 rounded px-1 py-0.5 text-black bg-white`}
                                  ...
                                  <div>
                                    <label className="block font-medium text-black mb-0.5">Max Value</label>
                                    <input
                                      type="number"
                                      step="0.01"
                                      className={`w-full text-[${Math.max(6, fontSize - 2)}px] border border-gray-300 rounded px-1 py-0.5 text-black bg-white`}
                              ...
                              {(deeperField.field_types || []).includes(FieldType.BUTTON) && (
                                <div className={`mb-1 p-1 bg-gray-100 text-black rounded text-[${Math.max(6, fontSize - 2)}px] border border-gray-300`}>
                                  <label className="block font-medium text-black mb-0.5">Button Options</label>
                              ...
                              {(deeperField.field_types || []).includes(FieldType.SIGNATURE) && (
                                <div className={`mb-1 p-1 bg-gray-100 text-black rounded text-[${Math.max(6, fontSize - 2)}px] border border-gray-300`}>
                                  <label className="block font-medium text-black mb-0.5">Signature Settings</label>
                                  <div className="space-y-0.5">
  ...
  // Recursive function to prepare field for save (complete replacement of field_options)
  const prepareFieldForSave = (field: any): any => {
    const preparedField = { ...field };
    
    // Build complete field_options from scratch (not merge, but replace)
    const newFieldOptions: any = {};
    
    // Copy existing field_options properties
    if (preparedField.field_options) {
      Object.keys(preparedField.field_options).forEach(key => {
        // Only copy if value is not undefined/null
        if (preparedField.field_options[key] !== undefined && preparedField.field_options[key] !== null) {
          newFieldOptions[key] = preparedField.field_options[key];
        }
      });
    }
    
    // Add conditional logic to field_options
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Fields Data (before prepare):', fields);
    ...
    try {
      // Prepare fields: merge conditional logic into field_options
      const preparedFields = fields.map(field => prepareFieldForSave(field));
      console.log('Fields Data (after prepare):', preparedFields);
      ...
      const result = await formsAPI.updateForm(formId, {
        form_name: formName,
        description,
        fields: preparedFields,
      });
      ...
      if (error.response) {
        ...
        if (error.response.status === 401) {
          ...
          errorMessage = error.response.data?.detail || 'Invalid form data.';
